...
- ✅ **All 268 tests passing** (266 Vitest, 2 Python pytest)
...
- ✅ **Release artifacts published**: GitHub release v0.3.0-alpha.5, container at `ghcr.io/xagi-lab/melra:alpha`
- ✅ **npm packages published**: all 14 packages live under the `@melra` scope on the `alpha` dist-tag, each with provenance attestation
- ✅ **CHANGELOG.md** updated with 0.3.0-alpha.5 section
...
**Action:** Install `npx @melra/cli@alpha` or download the `v0.3.0-alpha.5` release artifact, configure each client per docs/INSTALLATION.md, verify discovery/plan/execute/receipt cycle.
...
**Current state:** Threat model reviewed for 0.3.0-alpha.5, no independent audit yet.
