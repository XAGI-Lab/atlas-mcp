...
- ✅ **All 306 tests passing** (304 Vitest, 2 Python pytest)
...
- ✅ **Release artifacts published**: GitHub release v0.3.0-alpha.6, container at `ghcr.io/xagi-lab/melra:alpha`
- ✅ **npm packages published**: all 14 packages live under the `@melra` scope on the `alpha` dist-tag, each with provenance attestation
- ✅ **CHANGELOG.md** updated with 0.3.0-alpha.6 section
...
**Action:** Install `npx @melra/cli@alpha` or download the `v0.3.0-alpha.6` release artifact, configure each client per docs/INSTALLATION.md, verify discovery/plan/execute/receipt cycle.
...
**Current state:** Threat model reviewed for 0.3.0-alpha.6, no independent audit yet.
